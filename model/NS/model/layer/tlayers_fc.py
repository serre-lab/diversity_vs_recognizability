...
import numpy as np
from .layers_fc import PostPool, f_std, FCResBlock, Conv2d3x3
...
class AttentiveStatistiC(nn.Module):
    ...
    def __init__(
        ...
        print(f'No. of heads in transformer block: {self.n_head}')

        self.postpool = PostPool(self.hid_dim, self.c_dim, self.activation)
        self.aggregation_module = TBlock(self.hid_dim, n_head = self.n_head)
