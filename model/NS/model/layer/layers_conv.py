...
class Conv2d3x3(nn.Module):
    def __init__(
        ...
        self.conv = nn.Conv2d(
            in_channels, out_channels, kernel_size=kernel_size, padding=pad, stride=stride
        )
...
class SharedConvEncoder(nn.Module):
    def __init__(self, nc: int, activation: nn, img_dim: int =50, img_ch: int = 1):
    ...
    def __init__(
        ...
        self.conv_layers = nn.ModuleList(
            [
                ...
                Conv2d3x3(nc // 2, nc // 4),
                Conv2d3x3(nc // 4, nc // 4, kernel_size=4, pad=pad),
        ...
        self.conv_final = Conv2d1x1(32, out_ch)
