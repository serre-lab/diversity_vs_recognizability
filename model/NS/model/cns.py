...
class CNS(NS):
    ...
    def loss(
        self, out: dict, weight: float = 1.0, free_bits: bool = False, beta = 1.0, bs=None, ns=None
    ...
    def compute_mll(
        ...
        kl_c = (
            (cqd[0].log_prob(cqs[0]) - cpd[0].log_prob(cqs[0])).sum(-1).sum(-1).sum(-1)
        )
