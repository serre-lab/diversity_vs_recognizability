...
from .layer.layers_fc import (PosteriorZ, PriorZ, ObservationDecoder,
...
class NS(BaseModel):
    ...
    def top_down_stochastic_inference(self, 
        ...
        for td in range(self.n_stochastic): # L, L-1,...,1
            ...
            out["zqd"].append(zqd)
            out["zqs"].append(zq)
            out["zq_params"].append([zq_mean, zq_logvar])
        ...
        for td in range(self.n_stochastic):
            ...
            zq = out["zqs"][td]
            out["zpd"].append(zpd)
            out["zp_params"].append([zp_mean, zp_logvar])
        return out
    ...
    def forward(self, x: torch.Tensor):
        ...
        out["x"] = x.view(bs, ns, 1,50,50)
        out["xp"] = xp.view(bs, ns, 1,50,50)
        out["x_rec"] = xp.view(bs, ns, 1,50,50)
        out["mse"] = F.mse_loss(out["x"], out["xp"], reduction='sum')
        return out
    ...
    def bernoulli_log_likelihood(self, x, p, clip=True, eps=1e-6):
        if clip:
    ...
    def loss(self, 
             out: dict, 
             weight: float=1.0,
             free_bits: bool = False, beta = 1.0, bs=None, ns=None
        ...
        # Loss function adapted from https://github.com/comRamona/Neural-Statistician/blob/7ff41fdf97e0e4ca3a335901d107f6de0edb5481/omniglot/omnimodel.py#L135
        ...
        x = out["x"].view(-1, 1,50,50)
        xp = out["xp"].view(-1, 1,50,50)
        
        # likelihood for observation model
        logpx = self.bernoulli_log_likelihood(x, xp) / den
        ...
        # c_mean, c_logvar = out["c_mean"], out["c_logvar"]
        # kl_c = self.kl_diagnormal_stdnormal(c_mean, c_logvar)
        for l in range(self.n_stochastic):
            kl_z += td.kl_divergence(zqd[l], zpd[l]).sum()
            # kl_z += self.kl_diagnormal_diagnormal(out["zp_params"][l][0],
            #                                         out["zp_params"][l][1],
            #                                         out["zq_params"][l][0],
            #                                         out["zq_params"][l][1])
        ...
        w_vlb = ( (weight * logpx) - beta*(kl / weight) )
        loss = - w_vlb
    ...
    def generate(self, n_samples, exemplar=None):
