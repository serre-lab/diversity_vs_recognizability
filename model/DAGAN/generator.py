...
class Generator(nn.Module):
    ...
    def forward(self, x, z):
        ...
        return self.sigmoid(curr_input)


### ResNet

class _EncoderBlockResNet(nn.Module):
    def __init__(
        self, pre_channels, in_channels, out_channels, num_layers, dropout_rate=0.0
    ):
        super().__init__()
...
class _DecoderBlockResNet(nn.Module):
    def __init__(
        self,
        pre_channels,
        in_channels,
...
class ResNetGenerator(nn.Module):
    def __init__(self, dim, channels, dropout_rate=0.0, z_dim=100, l_size=64):
        super().__init__()
        self.dim = dim
        self.z_dim = z_dim
        self.channels = channels
        self.layer_sizes = [l_size, l_size, l_size*2, l_size*2]
        ...
        for i in range(self.U_depth + 1):
            ...
            # Input from injected noise
            if i < self.noise_encoders:
                in_channels = 128 + self.z_channels[i]
